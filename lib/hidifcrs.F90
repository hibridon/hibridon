...
!  added capability to treat cases with j12 but having twomol=F
!
!  previous revision:  26-jun-2017 some half-int. mol-mol debugging
!                                 by j.klos
!  current revision:  2-jun-2021 by p.dagdigian
!---------------------------------------------------------------*
!                                                               *
...
call openf(smt_unit,xnam1(1:lenx),'tu',0)
!
...
if (mflag) then
   ...
   call openf(rho_unit,xnam3(1:lenx),'sf',0)
endif
...
!
call rdhead(smt_unit,cdate1,ered1,rmu1,csflg1,flghf1, &
   flgsu1, &
...
!
write(dcs_unit, 41) xnam1,cdate1,label,potnam,econv*ered1,xmconv*rmu1
if (mflag) &
     write(rho_unit, 41) xnam1,cdate1,label,potnam,econv*ered1,xmconv*rmu1
...
!
!.....check whether initial level exists
!
do 60 j=1,nlevel
...
goto 500
!
!.....ckeck whether second level exists
!
! added for negative nnout by moonbong yang 2-8-95
...
!.....print header
!
160 write(dcs_unit, 171) j1,in1,j2,in2,jtot1,jtot2,ca,1.8897*ca, &
             econv*ecol,econv*ecol/8.065465
if (mflag) write(rho_unit, 171) j1,in1,j2,in2,jtot1,jtot2,ca,1.8897*ca, &
             econv*ecol,econv*ecol/8.065465
write(6,170) j1,in1,j2,in2,jtot1,jtot2,ca,1.8897*ca, &
             econv*ecol,econv*ecol/8.065465
170 format('    TRANSITION:  J1=',i4,' IN1=',i4, &
        '  ->  J2=',i4,' IN2=',i4/ &
        '    SUMMING PARTIAL WAVES FROM JTOT = ',i1, &
        ' TO',i4/ &
        '    WAVE VECTOR IN INITIAL CHANNEL:  ',f10.4,' Bohr^(-1)', &
        '; ',f10.4,' Angstroms^(-1)',/ &
...
176 format('    ROTATIONAL DENSITY MATRIX ELEMENTS SAVED IN FILE: ', &
       (a))
!
!.....determine number of angles per batch
!
   l2max=jtot2+j2+1
   mlmax=j1+j2+1
   ideg1=2*j1+1
...
!  molecule-molecule case
       l2max = jtot2 + j1/10 + mod(j1, 10) &
         + j2/10 + mod(j2, 10) + 1
       mlmax = j1/10 + mod(j1, 10) &
         + j2/10 + mod(j2, 10) + 1     
     else
!  special atom-molecule collisions
       l2max = jtot2 + j1 + in1 + j2 + in2 + 1
       mlmax = j1 + in1 + j2 + in2 + 1
     end if
   end if
   if(flaghf) then
...
!
!     Allocate and zero out m-dependent integral cross sections
...
! jk Added for half-integer molecule-molecule
    if(flaghf) then
...
!  special atom-molecule case
    iq1min = 1
    iq1max = (2 * j1 + 1) * (2 * in1 + 1) &
         * (2 * j2 + 1) * (2 * in2 + 1) * nangle
!      half integer
    if(flaghf) then
      iq1max = (2 * j1 + 1) * (2 * in1 + 2) &
         * (2 * j2 + 1) * (2 * in2 + 2) * nangle
    endif
    iq2min = 1
    iq2max = 1
    iq3 = 1
...
!
!.....print differential cross sections for this batch of angles
!
310 angle=ang1
...
!      if (j2.ge.1) then
   do 342 mj2=-j2p,j2-2
      do 341 i=1,nangle
!            print *, ' fm1m2(mj2,i),fm1m2(mj2+2,i):  ',
!     :                   fm1m2(mj2,i),fm1m2(mj2+2,i)
      term= &
...
!            print *, 'mj1,mj2,term,a22fak(mj2):  ',
!     :     mj1,mj2,term,a22fak(mj2)
341       continue
342    continue
  ...
  ('%   DIAGONAL (M-FINAL) DENSITY MATRIX (FIRST ROW)', &
   ...
   if (iprint) &
        ...
        angle,s(i)*fak,sm(i)/s(i),sm6(i)/s(i),a22(i)/s(i), &
           a21(i)/s(i),a11(i)/s(i)
   write(dcs_unit, 350) angle,s(i)*fak,sm(i)/s(i),sm6(i)/s(i),a22(i)/s(i), &
        a21(i)/s(i),a11(i)/s(i)
...
360    format &
  ('    DEGENERACY AVERAGED STERIC DXSC''S: HEADS AND TAILS')
   ...
   write(dcs_unit, 361) angle,0.5d0*s(i)*fak,0.5d0*sm(i)*fak
...
ang1=angle-dang
!
!.....loop back if not all required angles are finished
!
...
if (mflag) then
   ...
   write(dcs_unit, 376) ang0,dang,ang2
375    format ( &
...
! jk Added projections and degeneracy factor for half-integer molecule-molecule
  if (flaghf) then
    j1_ip=j1_i+1
    j1_fp=j1_f+1
...
1348   format(/'%  DEGENERACY AVERAGED DXSC (ANG^2/SR)'/'dcs = [')
  angle = ang1
  do 1351 i = 1, nangle
    write(dcs_unit, 1350) angle, s(i)*fak
...
!  compute degeneracy averaged cross sections only
  ...
  j2_fp = j2_f
  ideg1 = (2*j1_i + 1)*(2*j2_i + 1)
!  add projections and degeneracy factor for half-integer special atom-molecule
  if (flaghf) then
   j2_ip=j2_i+1
   j2_fp=j2_f+1
   ideg1 = (2*j1_i + 1)*(2*j2_i + 2)
  endif
! 
  fak = ang2c/(ideg1*ca**2)
  do 3500 mj1_i = -j1_i, j1_i
  do 3500 mj2_i = -j2_ip, j2_i
  do 3500 mj1_f = -j1_f, j1_f
  do 3500 mj2_f = -j2_fp, j2_f
    do 3400 i = 1, nangle
      ii =ii + 1
      dsigterm = dreal(q(ii, 1, 1) * conjg(q(ii, 1, 1)))
      s(i) = s(i) + dsigterm
...
!
  write(dcs_unit, 3348)
  if (iprint) write(6,3347)
3347   format(/'   DEGENERACY AVERAGED DXSC (ANG^2/SR)')
3348   format(/'%  DEGENERACY AVERAGED DXSC (ANG^2/SR)'/'dcs = [')
  angle = ang1
  do 3351 i = 1, nangle
    write(dcs_unit, 3350) angle, s(i)*fak
    if (iprint) write(6,1350) angle, s(i)*fak
3350     format(1x,f7.2,1g15.4)
    angle = angle + dang
3351   continue
end if
!
...
!     by q. ma
!  included option for molecule-molecule half-integer bases
!  added capability to treat cases with j12
!
...
!  special atom-molecule cases
!  note:  half integer spin applies to the atom (collider 2)
    j1_i=j1
    j2_i=in1
    j2_ip=j2_i + 1
    j1_f=j2
...
elseif (is_twomol(ibasty)) then
  ...
  xj1_i = dble(j1_i)
  xj2_i = dble(j2_i) + spin
  xj1_f = dble(j1_f)
  xj2_f = dble(j2_f) + spin
end if
!
llmax = 0
...
!  special atom-molecule cases
  ii = 0
  do 3400 mj1_i = -j1_i, j1_i
  do 3400 mj2_i = -j2_ip, j2_i
    xmj1_i = mj1_i
    xmj2_i = dble(mj2_i) + spin
    mj12_i = mj1_i + mj2_i
    xmj12_i = xmj1_i + xmj2_i
!
    do 3070 ll = 1, llmax
...
3070     continue
!
    do 3300 mj1_f = -j1_f, j1_f
    do 3300 mj2_f = -j2_fp, j2_f
      xmj1_f = mj1_f
      xmj2_f = dble(mj2_f) + spin
      mj12_f = mj1_f + mj2_f
      xmj12_f = xmj1_f + xmj2_f
      ml2 = mj12_i - mj12_f
      xml2 = ml2
...
!  spherical harmonics y  (theta,0) for  m .le. l .le. lmax
!
!  -------------------------------------------------------------------
!  variables in call list:
!    m:     magnetic projection quantum number
!    lmax:  maximum value of l (minimum value of l is m)
!    theta: polar angle
!    yr:    on return: array containing real part of spherical harmonic
!    results in yr(l*incy+1) for given l
!    for l values less than m zero is returned
!
!  -------------------------------------------------------------------
implicit none
integer, intent(in) :: m
...
real(8), intent(out) :: yr(lmax*incy)
integer, intent(in) :: incy

!     fpi = sqrt(1 / 4 * pi)
real(8), parameter :: fpi = 2.820947917738781d-01
real(8) :: a, fact
integer :: l, mm, ll
mm = iabs(m)
!  determine array of associated legendre polynomials
call plm(mm,lmax,theta,yr,incy)
!
!                      (l-mm)!  mm
!  on return:  yr(j) = ------- p        [cos(theta)]
!                      (l+mm)!  l=mm+j-1
if (m >= 0) then
!  here for positive m
      a = (( - 1.d0) ** m) * fpi
