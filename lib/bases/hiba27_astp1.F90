...
! ----------------------------------------------------------------------
subroutine baastp1 (bqs, jhold, ehold, ishold, nlevel, &
...
!  lb is the actual value of lambda
    ...
    do icol = 1, n
      do irow = icol, n
        lrow = bqs%lq(irow)
        if (csflag) lrow = nu
        call vlmatp1 (bqs%jq(irow), lrow, bqs%jq(icol), bqs%lq(icol), jtot, &
...
integer, pointer, save :: nterm, numpot, iop, jmax
