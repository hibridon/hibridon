...
The MRCRS command must have been preceded by the command [TENXSC](TENXSC), which creates the table of tensor cross sections used.
...
The calculated m-state resolved cross sections are printed out to the [file](Files) Job{ienerg}.mcs as well as to the standard output