...
⚠️ In order to create the `<jobname>.wfu` file, the previous run must have been done with the flags [`WAVEFL`](WAVEFL) and [`AIRYFL`](AIRYFL-and-PRAIRY) both set `.TRUE.`. In that run, [`WRSMAT`](PRSMAT-and-WRSMAT.md) controls whether information other than the adiabatic energies, *e.g.* the transformation matrices, will be written to the `wfu` file. If [`WRSMAT`](PRSMAT-and-WRSMAT.md) is set to FALSE, the `wfu` file will be much smaller but cannot be used for [`PSI`](PSI.md) or [`FLUX`](FLUX.md) calculations.
