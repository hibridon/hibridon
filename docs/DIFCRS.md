...
- {jobnam} the jobname under which the S matrices have been stored {jobname}n.smt. Here n denotes the value of the parameter ienerg (see below). These S matrices must have been previously generated using  
  - [`JLPAR`](JLPAR) = 0 (this generates S matrices for both parities)
  - [`JTOT1`](JTOT) = 1 (ensuring the determination of S-matrices at every partial wave)
  - [`WRSMAT`](PRSMAT-and-WRSMAT) = .true. (ensuring that the S-matrices are written to file {jobname}.smt).
...
- ienerg  the cardinal value of the [energy](Energy) for which the differential cross section is computed. i.e. if ienerg = 2, then the second energy S matrices [{jobname}2.smt] are used

- jtotend   the maximum value of $\mathbf{J}_{\rm tot}$ included in determining the scattering amplitude. The value of jtotend can not be larger than the variable [JTOT2](JTOT) used in the initial calculation
