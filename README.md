...
```bash
 git clone https://github.com/hibridon/hibridon.git ~/hib_src && mkdir -p ~/hib_build && cd ~/hib_build && cmake -DCMAKE_Fortran_COMPILER=gfortran -DBUILD_TESTING=ON ~/hib_build && make && ctest -L coverage
```
