...
!  in xz plane.
!
! Wrapper inherited from pot_ch3he_ccsdt.f  (q. ma, 20-jun-2011)
!
...
! LWORK, LIWORK: size of working (temp) arrays
...
! This is a C/gsl function that calculate Ylm terms
!
double precision PI, DEG, S4PI
parameter (PI=3.14159265358979323846d0, DEG=PI/180d0, &
S4PI=2d0*dsqrt(PI))
...
!
! Coefficients for Ylm terms - all (lambda,mu) combinations of lambda<=9,
!   with mu a multiple of 3.
...
ylm = YLMCAL
! Get potential values at distance r
do i = 1, NTHETA
 do j = 1, NPHI
  ind = NTHETA * (j - 1) + i
...
vv0 = vsp(1) / S4PI
! Long-range interactions have been taken care of in potcm function
return
end
!
! ---------------------------------------------------
...
!c potential subroutine for NH3-He, JCP, 114, 8836(2001)
!c the pes of JCP is in Cartesian coordinates
!c do the coordinate transformation in Body Fixed frame, origin is
!c on the center of mass of NH3
!c Aug. 07, 2007
!c -----------------Atomic Units------------------
!c inversion angle is fixed at experimental equilibrium geometry
! alpha_e=112.14, r_e=1.9132 bohr
!c
...
!cc-------------------------------------------------------------
!cc JCP, 114, 8836(2001)

double precision function pot4(x,y,z,theta)
!
! Calculates He-NH3 intermolecular potential in terms of Cartesian
! coordinates x, y, z (Bohr) of He relative to N as centre, with the NH3
! symmetry axis being the z axis, and one H atom lying in the zx plane at
! negative z and positive x.  The angle theta is the NH3 umbrella angle
! (Radians), and theta=pi/2 when the NH3 is planar.  The intramolecular
! distortion energy of NH3 is not included.
! 
implicit double precision(a-h,o-z)
...
common/cnh3he/x1,y1,z1,x2,y2,z2,x3,y3,z3, &
 p1,p2,p3,p4,p5,p6,p7,p8,b,x4,y4,z4,x5,y5,z5,x6,y6,z6, &
 qn1,qn2,qn3,qn4,qn5,qn6,qn7,qn8,qn9,qn10,qh1,qh2,qh3, &
 qh4,qh5,qh6,qc1,qc2,alpha,r
save
data p11,p21,p31,p41,p51,p61,p71,p81,p12,p22,p32,p42,p52,p62,p72 &
,p82,p13,p23,p33,p43,p53,p63,p73,p83,h1,h2,h3,h4/ &
 11.6523488375684d0,-2.98158118204410d0,1.49393389265970d0, &
 17.2811199030927d0,379.968752899844d0,-17.3581436591624d0, &
 0.261720480745098d0,29.5523802147692d0,-7.12983123895760d0, &
 6.60000388701621d0,-0.404967288493762d0,78.7087154209477d0, &
 -534.987689713811d0,-62.3083854882989d0,1.93535982924854d0, &
 -10.1837463629195d0,-1.09198084573485d0,1.75627013552562d0, &
 -1.40704330357101d0,286.638844403380d0,-223.587329773642d0, &
 118.110532011749d0,0.668417880283956d0,-6.22354215723589d0, &
 1.8726d0,0.224d0,-0.18d0,0.45d0/
data qn1a,qn2a,qn3a,qn4a,qn5a,qn6a,qn7a,qn8a,qn9a,qn10a,qh1a,qh2a, &
 qh3a,qh4a,qh5a,qh6a,qc1a,qc2a,qn1b,qn2b,qn3b,qn4b,qn5b,qn6b/ &
 105.358582685727d0,-93.5137650766694d0,-20.0556731797762d0, &
 53.9601454589709d0,-13.3141673258754d0,-26.7620176122042d0, &
 26.6988260179006d0,4.76344131933886d0,-13.4722471409789d0, &
 3.55461655537341d0,77.7307451245117d0,83.4124804317485d0, &
 10.8174097511536d0,-22.9172044461988d0,-22.8113636404374d0, &
 -2.74141173554461d0,-103.846480504968d0,31.2016079201890d0, &
 -124.151847166268d0,417.282168854078d0,22.9881057315202d0, &
 -254.773867022760d0,-18.5021336999981d0,34.8299600687467d0/
data qn7b,qn8b,qn9b,qn10b,qh1b,qh2b,qh3b,qh4b,qh5b,qh6b,qc1b,qc2b, &
 qn1c,qn2c,qn3c,qn4c,qn5c,qn6c,qn7c,qn8c,qn9c,qn10c,qh1c,qh2c/ &
 -96.5301259714376d0,-5.06116647156490d0,62.5367249014338d0, &
 -5.03407376699773d0,-137.948336849227d0,-162.002983441311d0, &
 -22.7412630993916d0,33.5154303928627d0,35.3285466381656d0, &
 4.55272224376798d0,180.648860035816d0,-45.1230870070718d0, &
 331.449373618478d0,-597.207250158190d0,-43.2352558277175d0, &
 274.524585476137d0,203.911154564190d0,-72.3569784341317d0, &
 120.361515733561d0,7.26811312052723d0,-64.9599619089990d0, &
 -25.1531925783774d0,345.667422849100d0,411.949407885026d0/
data qh3c,qh4c,qh5c,qh6c,qc1c,qc2c/ &
 60.0114831548612d0,-72.5666681773184d0,-80.7346698141146d0, &
 -11.0424451043878d0,-441.365662544351d0,94.6162278421202d0/
b=2.7d0
alpha=1.89d0
c=cos(t)
c2=c*c
