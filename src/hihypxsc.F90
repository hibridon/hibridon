...
!
!     modified to treat atom-molecule collisions with two nuclear spins
!     see Lara-Moreno, Stoecklin, and Halvick, MNRAS 507, 4086 (2021).
!     In this case, nucspin = 100*2*I1 + 2*i2
!
!     current revision:  22-dec-2021 by p. dagdigian
!     -------------------------------------------------------------
...
do 60 i=1, nlevel
   ...
   if (ij1.lt.j1min .or. ij1.gt.j2max) goto 60
...
!  formalism taken from Lara-Moreno et al., MNRAS (2021)
!
        fhspin = 0.d0
        nuc1 = 2 * finuc
        if (flaghf .and. nuc1.eq.2*(nuc1/2) .or. &
            ...
            fhspin = 0.5d0
        fhspin2 = 0.d0
        if (mod(f2nuc,1.d0) .ne. 0.d0) fhspin2 = 0.5d0
        nlevlh2 = 0
        do 160 i=1, nlevelh
...
!  equals F2 - 0.5 if F2 is half-integer
          ...
          write(6,877) nlevlh2,jlevh2(nlevlh2),inlevh2(nlevlh2), &
           flevh1(nlevlh2),flevh2(nlevlh2),elevh2(nlevlh2) * econv
877     format('  nlevlh2=',3i5,2f5.1,f10.3)
...
1334      format(' Computing partial wave K_tot =',f5.1, &
        ...
        write(6,3322) xj,xf,xf2,xjp,xfp,xf2p
3322  format('***   j=',f5.1,' F1=',f5.1,' F2=',f5.1, &
         3x,'jp=',f5.1,' F1P=',f5.1,' F2p=',f5.1)
...
!     ftot = jtot + nucspin
                ...
                do ir = irmin, irmax
                  r  = ir + fhspin2
                 write(6,432) r,rmin,rmax
432             format('     Computing K_tot - I2 =',f5.1, &
                    '  ('2f6.1,')')
                  qmin = abs(r - finuc)
                  qmax = r + finuc
                  iqmin = qmin
...
!     convert S-matrix element to T-matrix element
           ...
           write(6,878) i,ii,irow,icol,xl,xlp,sr(jtot,jlpt,is),si(jtot,jlpt,is)
878      format(2i3,2x,2i3,2x,'  L=',f5.1,' LP=',f5.1,4x,2f12.6)
...
!     next statement for diagonal T-matrix element
        ...
        write(6,4322) xj,xf,xf2,xl,xjp,xfp,xf2p,xlp,xjtot,xftot
4322  format('      ',4f5.1,3x,4f5.1,2x,'  jtot=',f5.1, &
         '  ftot=',f5.1)
         ...
         iskip2=0
         if (iskip2 .ne. 1) then
         write(6,442) f2nuc,xf,xf2,xl,xftot,r, &
                xf6j(f2nuc,xf,xf2,xl,xftot,r), &
             f2nuc,xfp,xf2p,xlp,xftot,r, &
        xf6j(f2nuc,xfp,xf2p,xlp,xftot,r), &
             finuc,xj,xf,xl,r,xjtot, &
        xf6j(finuc,xj,xf,xl,r,xjtot), &
             finuc,xjp,xfp,xlp,r,xjtot, &
        xf6j(finuc,xjp,xfp,xlp,r,xjtot)
  
442      format(3f4.1' /',3f4.1,f8.4/3f4.1,' /',3f4.1, &
           f8.4/3f4.1,' /',3f4.1,f8.4/3f4.1,' /',3f4.1,f8.4/)
         end if
          ...
          do 185 ii=1,nlevlh2
            ...
            xf2p = flevh2(ii)
             if (sigma(i,ii).ne.0.d0) then
              write(6,1006) ee,xj,inlevh(i),xf,xf2,&
                xjp,inlevh(ii),xfp,xf2p,sigma(i,ii)
              write(11,1006) ee,xj,inlevh(i),xf,xf2,&
                xjp,inlevh(ii),xfp,xf2p,sigma(i,ii)
