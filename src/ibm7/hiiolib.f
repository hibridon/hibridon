...
*  current revision date:  24-feb-1998 by mha
* ------------------------------------------------------------------
*  line 1:
...
*    spac:        step size in the logd integration
*    if logdfl=.false., 1. then rendld can be given any value, it will be set
...
*    wrxsec, xsecwr:
*                if either of these variables is .true., then unit=15 to
*                           unit=(14+nerg) are opened for storage of some
*                           input data and degeneracy averaged integral
*                           cross sections
*                if wrxsec = .true., then unit=15 to unit=(14+nerg) are opened
*                                    as permanent files with filenames
*                                    xsec1, xsec2, xsec3, ... , xsecn
*                                    where n = nerg
*                          = .false., then unit=15 to unit=(14+nerg) are opene
...
*  --------------------------------------------------------------------
     ...
     :                boundc
      ...
      common /coselb/ ibasty
      if (nerg .gt. 1) then
*  check to see if nerg .le. 10
        if (nerg .gt. 10) then
          write (9, 10) nerg
          write (6, 10) nerg
10        format (/' *** NERG =', i2,' > 10; ABORT ***')
          call exit
        end if
...
*     photodissociation calculation or wavefunction calculation
      if (wrxsec .or. xsecwr .or. partw .or. wrpart) then
        if (.not. wavefl .and. .not. photof) then
           ...
           if (lenj .gt. 10) then
             call gennam (xname, jobnam(1:10), 0, 'sav', lenx)
           else
...
* open direct access file for storage of wavefunction
      if (wavefl.and. .not. boundc) then
        ...
        if (lenj .eq. 0) lenj=40
        if (lenj .gt. 10) then
          call gennam (xname, jobnam(1:10), 0, 'wfu', lenx)
        else
...
c;      character*12 stat
c;      character*(*) name
c;      common/disc/ ipos(98),iun(98),nam(98),iostat(98),icatf(98)
...
10    format('t',i2.2,i5.5,'.tmp  ')
      ...
      character*15 namx
      common/disc/ ipos(98),iun(98),nam(98),iostat(98),icatf(98)
      common/clseg/ lseg
