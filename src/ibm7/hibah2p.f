...
*  current revision date:  13-jun-2002 by mha
* --------------------------------------------------------------------
...
235         continue
        ...
        elseif (.not.ihomo) then
          if (flaghf) then
            write (6, 255)
            write (9, 255)
255         format(/'   N   JA(IS)  JMOL  K(J)   CENT    EINT(CM-1)')
            do 265  i = 1, n
              xs=is(i)+half
              icent=cent(i)
              write (6, 260) i,xs,isc1(i),j(i),icent,eint(i)*econv
              write (9, 260) i,xs,isc1(i),j(i),icent,eint(i)*econv
260           format (i4, f8.1,3i6, f14.3)
265         continue
          else
            write (6, 270)
            write (9, 270)
