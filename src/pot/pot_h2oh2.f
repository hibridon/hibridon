c     pot routine for the interaction of H2O with H2
c     computed by Valiron et al. (2009), coefficients provided by A. Faure
c     23-oct-2019
c
...
c     current revision:  28-oct-2019 (p. dagdigian)
c     ------------------------------------------------------------------
...
c
     ...
     $  xmconv=0.0005485799094979479d0, sq4pi=3.544907701811032d0)
      end module mod_h2oh2
c     ----------------------------------------------------------------- 
c     loapot subroutine loads pot parameters for H2O-H2 interaction
c     ----------------------------------------------------------------- 
...
C     the time.
      ...
      pot_name = 'H2O-H2 Valiron(2009) PES'
...
C     Main program for makepot
      ...
      common /covvl/ vvl
      real(8), dimension(1) :: vvl
      character(40), parameter :: data_file_name='pot_h2oh2.dat'
      real(8) :: r, vv0
      integer :: i, nv, nlam
...
C     ------------------------------------------------------------------
      ...
      common /covvl/ vvl
      real(8), dimension(1) :: vvl
      double precision vv0, r_inp, r, rmax, rsw, swfact, sw, vlr, vlast
...
c  long-range extrapolation
         rsw = 14.d0
         swfact = 1.0d0
         sw = 0.5d0 * (tanh(swfact * (r - rsw)) + 1.d0)
         if (iv .eq. 1) then
c  (0 0 0 0) term -- extrapolate as 1/R^6, using value at last R
           vlast = seval(nr1, rr1(nr1), rr1, coef1(1, iv), 
     $        spl_b1(1, iv), spl_c1(1, iv), spl_d1(1, iv))
           vlr = vlast * rr1(nr1)**6
           vvl(iv) = vvl(iv) * (1.d0 - sw) + sw * vlr / r**6
         elseif (iv .eq.7) then
c  (1 0 2 3) term -- extrapolate as 1/R^4
           vlast = seval(nr1, rr1(nr1), rr1, coef1(1, iv), 
     $        spl_b1(1, iv), spl_c1(1, iv), spl_d1(1, iv))
           vlr= vlast * rr1(nr1)**4
           vvl(iv) = vvl(iv) * (1.d0 - sw) + sw * vlr / r**4
         elseif (iv .eq. 22) then
c  (2 2 2 4) term -- extrapolate as 1/R^5
           vlast = seval(nr1, rr1(nr1), rr1, coef1(1, iv), 
     $        spl_b1(1, iv), spl_c1(1, iv), spl_d1(1, iv))
           vlr= vlast * rr1(nr1)**5
           vvl(iv) = vvl(iv) * (1.d0 - sw) + sw * vlr / r**5
         else
           vvl(iv) = vvl(iv) * (1.d0 - sw) 
         end if 
      end do
      return
      end subroutine pot
