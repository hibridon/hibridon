...
*
*  in this version,only the isotropic (V00) term is retained
*
...
* ------------------------------------------------------------------------
      ...
      econv=219474.6d0
      potnam='H-CH4 RCCSD(T)-F12a isotropic'
      print *, potnam
1     print *, 'R (bohr):'
      read (5, *, end=99) r
      call pot(vv0,r)
...
99    rr=3.5d0
      dr=0.5d0
      open (unit=12,file='hch4_iso_vlms.dat')
      write(12,109)
...
*
*  IN THIS VERSION, SET ALL ANISOTROPIC TERMS TO ZERO
         if (ilam .eq. 1) then
           v(ilam)=vvx
         else
           v(ilam) = 0.d0
