...
*
      ...
      potnam='Ar-CN(A,v=0) RCCSD(T) PES D. Bennett'
      print *, potnam
1      print *, ' r (bohr)'
      rshift=0.5
      xfact=0.8
...
99    rr=3.5d0
      dr=0.5d0
      open (unit=12,file='cnar_Av0_dbennett_vlms.txt')
      write(12,109)
...
* ------------------------------------------------------------------------
      ...
      common /pointsdif/rdif(nptdif),rh2dif(nptdif),thetadif(nptdif)
      common /coefdif/vcoefdif(nptdif)
      potnam='Ar-CN(A) RCCSD(T) PES D. Bennett'
      lammin(1)=1
      lammax(1)=10
      lammin(2)=2
      lammax(2)=10
...
* determine A' and A" potentials at angles
      do 100 i=1,11
        vsum(i)=vsum_3D(2.3455D0,r,thta(i),vcoefsum)
* don't compute vdif for colinear geometries
        if (i.ne.1 .and. i.ne.11) then
          vdif(i-1)=vdif_3D(2.3455D0,r,thta(i),vcoefdif)
