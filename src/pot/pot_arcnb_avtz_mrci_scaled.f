...
* ------------------------------------------------------------------------
      ...
      potnam='ALEXANDER CN(B)-Ar avtz+bondf scaled MRCI(D)'
      lammin(1)=1
      lammax(1)=9
      mproj(1)=0
      ntv(1)=1
...
*  Ar-CN(B) potential of Alexander
...
*    :    coefb(kmax,kang),xa(kmax),
*    :    worksp(5*(kmax+6)),
*    :    t(kmax+6),bcoef(kmax),inbv(kang)
...
* spline fit to obtain coefficients
      ...
      if (ispline.eq.0) then
        ind=0
        do ith=1,nang
* determine  initial spline coefficients
