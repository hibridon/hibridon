...
*   current revision date:  7-dec-2017 by pdagdigian
...
99    rr=2.6d0
      dr=0.25d0
      open (unit=12,file='okr_uccsdt_vlms_2state.txt')
      write(12,109)
...
* latest revision date:  14-apr-2017
* ----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      include "common/parbas"
      common /covvl/ vvl(3)
...
*  set vv0 term to zero
      vv0 = 0.d0
*  subtract asymptotic energy from singlet potentials
*      do ir = 1, 85
*        vl(ir) = vl(ir) - 1.5906640e+04
*      enddo
*
*  spline fits
      if (ifirst .eq. 0) then
...
*  1Sigma energy
        if (ilam.eq.1) then
          v(1) = vvx 
*  subtract asymptotic energy from 1Sigma potential energy
          v(1) = v(1) - 1.5906640e+04
        endif
*  3Pi energy
        if (ilam.eq.5) then
          v(2) = vvx
