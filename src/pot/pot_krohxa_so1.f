*system:  OH(A/X)-Kr
*
*  MULTIPLY S-O MATRIX ELEMENTS BY 10
*
...
*  Az matrix element
100   xomeg = float(iomeg) + half
      x = x * xf3j(xjp, xlamda, xj, -xomeg, zero, xomeg)
      goto 400
...
* current revision:  22-aug-2012 by p. dagdigian
* ----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
...
c  VNO  pot at r, output
     ...
     :  vvec_z(27,29), vvec_m(27,29), vvec_p(27,29), ifirst
      dimension vsp_az(29), vsp_aymax(29), vsp_aypax(29)
      dimension vv(1330), v(27,29)
...
*  read in spin-orbit matrix elements on first entry
      if (ifirst .eq. 0) then
*  Az
        open (unit=10,file='potdata/krohxa_az_lams.dat')
        do i=1,27
          read (10, *) (v(i,j), j=1,11)
        end do
        close(10)
        do j=1,29
          call dcopy(27,v(1,j),1,vvec_z(1,j),1)
* vvec arrays contain potentials at all 27 points (rows) for each of the 29 
* theta values (columns)
* determine spline coefficients for all 29 theta values
          call spline(27,rr,vvec_z(1,j),a0_z(1,j),
     :      b0_z(1,j),c0_z(1,j))
        enddo
*  (Ay - Ax)/sqrt(2)
        open (unit=10,file='potdata/krohxa_aymax_lams.dat')
        do i=1,27
          read (10, *) (v(i,j), j=1,11)
        end do
        close(10)
        do j=1,29
          call dcopy(27,v(1,j),1,vvec_m(1,j),1)
* vvec contains potentials at all 27 points (rows) for each of the 29 
* theta values (columns)
* determine spline coefficients for all 29 theta values
          call spline(27,rr,vvec_m(1,j),a0_amx(1,j),
     :      b0_amx(1,j),c0_amx(1,j))
        enddo
*  (Ay + Ax)/sqrt(2)
        open (unit=10,file='potdata/krohxa_aypax_lams.dat')
        do i=1,27
          read (10, *) (v(i,j), j=1,9)
        end do
        close(10)
        do j=1,29
          call dcopy(27,v(1,j),1,vvec_p(1,j),1)
* vvec contains potentials at all 27 points (rows) for each of the 29 
* theta values (columns)
* determine spline coefficients for all 29 theta values
          call spline(27,rr,vvec_p(1,j),a0_apx(1,j),
...
* at all 29 theta values
      do j=1,29
        vsp_az(j)=seval(27,r,rr,vvec_z(1,j),a0_z(1,j),
     :    b0_z(1,j),c0_z(1,j))
        vsp_aymax(j)=seval(27,r,rr,vvec_m(1,j),a0_amx(1,j),
     :    b0_amx(1,j),c0_amx(1,j))
        vsp_aypax(j)=seval(27,r,rr,vvec_p(1,j),a0_apx(1,j),
     :    b0_apx(1,j),c0_apx(1,j))
      enddo
      return
      end   
