* System:  NH(c 1Pi)+Ar, modified ab initio MRCI+Q PES's
...
* --------------------------------------------------------------------------
      ...
      potnam='ALEXANDER-WERNER Ar-NH(c) mod_yang MRCI'
      lammin(1)=1
      lammax(1)=7
      lammin(2)=2
      lammax(2)=7
...
* author:  millard alexande
* latest revision date:  6-jan-1994
* ----------------------------------------------------------------------
     ...
     :          ddif(8),vap(8),va2p(8),
     :          d0(64),d2(36),aa(64)
      dimension kpvt(8),qraux(8),work(55),rsd(8),re(8)
      dimension depth(16),angle(8),p1(8),p2(8),p3(8),p4(8)

* rshift is factor for shift of atan midpoint in potential correction
* xfact is factor for additional potential correction
      common /cosysr/ xjunk(2),fact0,fact1,fact2,q,fact3,fact4
      common /covvl/ vvl(13)
...
* re for a state Vsum
      data re /
     : 6.1694, 6.2514, 6.5424, 7.2500,7.4207, 7.1685, 6.6334, 6.1706/

*     : 6.2263605d0, 6.2459680d0, 6.4809855d0, 6.9612032d0, 7.1134717d0, 
*     : 6.7141310d0, 6.3121589d0, 6.1974561d0,
*     : 6.2328603d0, 6.3880079d0, 6.7417395d0, 7.6381031d0, 7.7362775d0, 
*     : 7.5899714d0, 6.9849750d0, 6.2139823d0/
**angle
      data angle /
     : 0.d0, 15.d0, 30.d0, 60.d0, 90.d0, 120.d0, 150.d0, 180.d0/
...
* correct for differences in linear geometries
       ...
       bc=0.25*(dif0-dif180)
      do 110 i=1,8
        p1(i)=cos(angle(i)*pi/180.d0)
        p12=p1(i)*p1(i)
        p2(i)=(-1d0+3.d0*p12)/2.d0
        p3(i)=p1(i)*(-3.d0+5.d0*p12)/2.d0
        p4(i)=(3.d0-30.d0*p12+35.d0*p12*p12)/8.d0
        xfact=fact0+fact1*p1(i)+fact2*p2(i)

        vap(i)=vap(i)-ac-bc*p1(i)
        va2p(i)=va2p(i)+ac+bc*p1(i)
        fact=xfact*(tanh(alph*(r-re(i)+rshift))+1)
        vsum(i)=half*(va2p(i)+vap(i))
        vsum(i)=vsum(i)*(1+fact)
* don't compute vdif for colinear geometries
        if (i.ne.1 .and. i.ne.8) then
          fact=fact3*(tanh(alph*(r-re(i-1)+rshift))+1)
          vdif(i-1)=half*(va2p(i)-vap(i))
          vdif(i-1)=vdif(i-1)*(1+fact)
