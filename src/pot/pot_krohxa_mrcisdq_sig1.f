...
*
*  this is a more effucuent version of pot_krohxa_mrcisdq_sig.f,
*  which itself is a modification of pot_krohxa_mrcisdq_basgpi1.f
*  pot routine
*
...
*  rotational structure in both states taken as 1Sigma+
*  user-defined basis routine (ibasty=99) included in this file
*
...
*  current revision date;  16-apr-2012
*
*  subroutines called:  vlm1sg
*   
      subroutine driver
      ...
      potnam='OH(A/X)-Kr Quenching Klos MRCISDQ PESs-Sigma'
      print *, potnam
*  consider both v=0 and 1 vib levels of 2pi state
      numvib = 1
      ivibpi(1)=0
      ivibpi(2)=1
      nterm=4 + 3*(numvib - 1)
...
*    esg1:      energy of 1st sigma state in cm-1
*    bsg1:      rotational constant for 1st sigma state in cm-1
*    dsg1:      centrifugal distortion constant for 1st sigma state in cm-1
*    esg2:      energy of 2nd sigma state in cm-1
*    bsg2:      rotational constant for 2nd sigma state in cm-1
*    dsg2:      centrifugal distortion constant for 2nd sigma state in cm-1
...
*    nterm:    number of different diabatic potentials, this is 3 here
*    jmax1:    maximum rotational angular momentum in 1st sigma state
*    jmax2:    maximum rotational angular momentum in 2ns sigma state
...
*     :  jmax1, jmax2
*      common /cosysr/ isrcod, junkr, esg1, bsg1, dsg1, 
*     :                esg2, bsg2, dsg2
     ...
     :                flaghf, flagsu, ihomo, lpar(18)
      ...
      common /conlam/ nlam
*  arrays for storage of integrals of products of associated 
*  Legendre polynomials.  see description in header of bausr routine
      common /aleg/ pint(10,0:40,2001), nujc(0:40)
*
      save potfil
      include "common/comdot"
      irpot = 1
*  initialize nujc array
      do i = 0, 40
        nujc(i) = -1
      end do
...
*  read parameters for the two sigma states
*    jmax values
      read (8, *, err=888) (ispar(isicod+j),j=2,3)
...
*
      entry savusr (readpt)
*  save parameters for the two sigma states
      write (8, 105) (ispar(j),j=2,3),
     :   ' jmax1, jmax2'
      write (8, 203) (rspar(j),j=1,3),
     :   ' esg1, bsg1, dsg1'
      write (8, 203) (rspar(j),j=4,6),
     :   ' esg2, bsg2, dsg2'
105   format(2i4,t55,a)
203   format(3g14.6,t55,a)
      write (8, 300) potfil
300   format(a)
      return
      end
...
*  for collisional coupling of two 1Sigma states
...
*                 1st index:  lambda
*                 2nd index:  k
*                 3rd index:  (j,j'), isub = 50*j +j'
*               note:  integrals are not stored in the most efficient manner.
*               array is ~640K long, but the programmming is simple
*   nujc:       array to keep track of whether integrals calculated for
*               value of k
*  use description in header of bausr routine
...
* --------------------------------------------------------------------
      ...
      common /cosysr/ isrcod, junkr, rscod(maxpar)
     ...
     :          sc1(1), sc2(1), sc3(1),
     :          sc4(1), ishold(1), is(1)
      dimension esg(2), bsg(2), dsg(2), jmaxe(2)
*   econv is conversion factor from cm-1 to hartrees
*   xmconv is conversion factor from amu to atomic units
      data zero, one, two /0.d0, 1.d0, 2.d0/, ione /1/
      tzero = 1.d-12
      nterm = iscod(1)
      jmaxe(1) = iscod(2)
      jmaxe(2) = iscod(3)
      do ie = 1, 2
        esg(ie) = rscod(1+3*(ie-1))
        bsg(ie) = rscod(2+3*(ie-1))
        dsg(ie) = rscod(3+3*(ie-1))
      end do
...
20        format(/,' **  CC 1SIGMA-1SIGMA TRANSITIONS  ** RMU=', f9.4,
     :           '   E=',f10.3,'   JTOT=', i5, 2x,' JLPAR=',i2)
        else
          write (6,25) rmu * xmconv, ered * econv, jtot, nu
          write (9,25) rmu * xmconv, ered * econv, jtot, nu
25        format(/,' **  CS 1SIGMA-1SIGMA TRANSITIONS ** RMU=', f9.4,
     :           '   E=',f10.3,'   JTOT=', i5, 2x,' NU=',i2)
        end if
...
*  print out spectroscopic parameters for the states
        write(6,31) ' State    E',' B','D'
        write(9,31) ' State    E',' B','D'
31      format(/a,8x,a,10x,a,15x,a)
        do 40 ie=1,2
        write(6,35) ie,(rscod(jj+3*(ie-1)),jj=1,3)
40      write(9,35) ie,(rscod(jj+3*(ie-1)),jj=1,3)
35      format(1x,i3,2x,f12.3,f12.6,1pe14.5)
      end if
      n=0
      nskip = 1
      if (ihomo) nskip = 2
      do 120 ie = 1, 2
        ...
        brot = bsg(ie)
        drot = dsg(ie)
        do 115 ji = 0, jmaxe(ie), nskip
          jj1 = ji * (ji + 1)
          ee = en + brot * jj1 - drot * jj1 ** 2 
          if (.not. csflag) then
...
*  if homonuclear diatomic, skip space is two
      do 200 ie = 1, 2
        ...
        brot = bsg(ie)
        drot = dsg(ie)
        do 195 ji = 0, jmaxe(ie), nskip
          jj1 = ji * (ji + 1)
          ee = en + brot * jj1 - drot * jj1 ** 2 
          nlevel = nlevel + 1
          ehold(nlevel) = ee / econv
          jhold(nlevel) = ji
          ishold(nlevel) = ie
195     continue
200   continue
...
*  ij is address of given v2 element in present v2 matrix
      ...
      if (ihomo) istep = 2
*
*  compute and store needed integrals of products of associated
*  Legendre polynomials
      if (csflag) then
*  integrals for cs calculation
        jmax = max(jmaxe(1),jmaxe(2))
        iterm = 3
        ...
        do il = 1,nlami
          ...
          do j1 = 0,jmax
            do j2 = 0, j1
              call legint(j1,nu,lb,ione,j2,nu,fint)
              isub = 50*j1 + j2 + 1
              pint(lb,nu,isub) = fint
              isub = 50*j2 + j1 + 1
              pint(lb,nu,isub) = fint
            end do
          end do
...
*  integrals for cc calculation here
        jmax = max(jmaxe(1),jmaxe(2))
        kmax = min(jmaxe(1),jmaxe(2),jtot)
        iterm = 3
        nlami = (lammax(iterm) - lammin(iterm))/istep + 1
        do il = 1,nlami
          lb = lammin(iterm) + (il - 1) * istep
          do j1 = 0,jmax
            do j2 = 0, j1
              do k = 0, kmax
                if (k.le.min(j1,j2)) then
                  call legint(j1,k,lb,ione,j2,k,fint)
                  isub = 50*j1 + j2 + 1
                  pint(lb,k,isub) = fint
                  isub = 50*j2 + j1 + 1
                pint(lb,k,isub) = fint
...
*  below for sigma potential Vsig1
        if (iterm.eq.1) ipot = 1
*  below for sigma potential Vsig2
        if (iterm.eq.2) ipot = 2
*  below for coupling potential V1
        if (iterm.eq.3) ipot = 3
499     nlami = (lammax(iterm) - lammin(iterm))/istep + 1
        do 500 il=1,nlami
          ...
          do 450 irow = icol, n
            ...
            goto (460, 470, 480), ipot
*  Vsig1
460         if (is(irow).eq.1 .and. is(icol).eq.1) then
              call vlm1sg(j(irow), lrow, j(icol), lcol,
     :          jtot, nu, lb, vee, csflag)
            end if
            goto 440
*  Vsig2
470         if (is(irow).eq.2 .and. is(icol).eq.2) then
              call vlm1sg(j(irow), lrow, j(icol), lcol,
     :          jtot, nu, lb, vee, csflag)
            end if
            goto 440
*  V1
480         if (is(irow).eq.1 .and. is(icol).eq.2 .or.
     :          is(irow).eq.2 .and. is(icol).eq.1) then
              call vlm1(j(irow), lrow, j(icol), lcol,
     :          jtot, nu, lb, vee, csflag)
            end if
...
* --------------------------------------------------------------------------
      subroutine vlm1(j1, l1, j2, l2, jtot, nu, lb, v, csflag)
* --------------------------------------------------------------------
*  subroutine to evaluate the angular coupling matrix element for collision-
*  induced transitions between two 1Sigma states induced by a V1 coupling
*  PES.  the form of V1 is:
*
*     V1(R,theta) = sum over lambda[ V1(lambda;R) * d(lambda; 10)(theta) ]
*
...
*
      ...
      xlb = lb
      if (.not. csflag) then
*  here for close-coupling, for the matrix element
        xl1 = l1
        xl2 = l2
        kmax = min(j1, j2, jtot)
        xnorm = sqrt((2.d0*xl1 + 1.d0) * (2.d0*xl2 + 1.d0))
        sum = zero
        do 10 k = 0, kmax
          xk = k
*  access integral of product of associated Legendre polynomials
*  from table
          isub = 50*j1 + j2 + 1
          x = pint(lb,k,isub)
          factr = xf3j(xj1,xl1,xjtot,xk,zero,-xk)
     :      * xf3j(xj2,xl2,xjtot,-xk,zero,xk)
          if (k.ne.0) factr = factr * 2.d0
          sum = sum + factr * x
10      continue
        v = sum * xnorm
      else
*  here for coupled-states
*
*  access integral of product of associated Legendre polynomials
*  from table
        isub = 50*j1 + j2 + 1
...
* --------------------------------------------------------------------------
      subroutine legint(l1,m1,l2,m2,l3,m3,fint)
* --------------------------------------------------------------------------
*  subroutine to compute numerically the integral of the product of three
*  associated legendre polynomials and multiply by prefactors to obtain
*  the body-frame matrix element:
*
*               < l1 m1 | d(l2)(m2,0) | l3 m3 >
*
*  written by p.dagdigian
*  cxurrent revision date:  12-apr-2012
*
*  subroutine used:
*       plgndr, dqtfg
* --------------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      dimension x(10000),fintg(10000)
*
*  if l1 + l2 + l3 is even, integral is zero
      lsum = l1 + l2 + l3
      if (lsum.eq.2*(lsum/2)) then
        fint = 0.d0
        go to 500
      end if
*
*  number of angular grid points
      ngrid = 50*(l1 + l2 + l3)
      if (ngrid.gt.10000) then
        write(6,*) 'ngrid in legint too large'
        stop
      end if
*
      dx = 2.d0/(ngrid - 1.d0)
      do 10 ith = 1, ngrid
        x(ith) = -1.d0 + dx * (ith - 1)
        fintg(ith) = plgndr(l1,m1,x(ith))
     :     * plgndr(l2,m2,x(ith))
     :     * plgndr(l3,m3,x(ith)) 
10    continue
      call dqtfg(x,fintg,zint,ngrid)
*
*  multiply by the prefactors
      factr = 1.d0
      if (m1.gt.0) then
        do i=1,2*m1
          factr = factr / (l1 + m1 - (i - 1))
        end do
      end if
      if (m2.gt.0) then
        do i=1,2*m2
          factr = factr / (l2 + m2 - (i - 1))
        end do
      end if
      if (m3.gt.0) then
        do i=1,2*m3
          factr = factr / (l3 + m3 - (i - 1))
        end do
      end if
      zint = zint * 0.5d0 * dsqrt((2.d0*l1 + 1.d0) 
     :   * (2.d0*l3 + 1.d0) * factr)
      fint = zint
500   return
      end
...
*  subroutine dqtfg
*
*  taken from IBM subroutine package
*
...
*  for first X level
      ...
      lammax(3)=9
      mproj(3)=1
*  for second X level, if included
      lammin(4)=0
      lammax(4)=10
      mproj(4)=0
      lammin(5)=1
...
*    vvl(z+1:a) expansion of v1 coupling two sigma states in d(l,0) terms
*    and more terms if numvib > 0
*
* authors:  paul dagdigian and jacek klos
* current revision:  16-mar-2012 by p. dagdigian
* ----------------------------------------------------------------------
      implicit double precision (a-h,o-z)
      logical ljunk, ihomo, csflag, lljunk
      include "common/parbas"
      dimension vsig(11), vpi(11), v1(10)
...
*
      ...
      do l=0,lmax
        do i=1,nangle
          ...
          if (l.ge.1 .and. l.lt.lmax
     :        .and.i.gt.1.and.i.lt.nangle) then
            index1=index1+1
            d1(index1)=DLM0(l,1,thta(i))
          endif
        enddo
      enddo
* set up values of Vsigma, VPi, and V1 PESs at angular grid
      do i=1,nangle
*   set Vsig as H22 A' diabat
        vsig(i)=VCI_H22(r,thta(i))
*   set Vpi as H11 A' diabat  
        vpi(i)=VCI_H11(r,thta(i))
*   set V1 as H12 A' off-diagonal coupling term
*   divide V1 by sqrt(2)
        v1(i-1)=VCI_H12(r,thta(i)) / sqrt(2.d0)
        if (r .gt. rmax) then
...
* then for v1
      ...
      call dcopy(9,x1,1,vvll(23),1)
*
* convert to hartree
      econv=1./219474.6
      call dscal(31,econv,vvll,1)
*
* vsig terms
      do i=1,lmax+1
        vvl(i)=vvll(i)
