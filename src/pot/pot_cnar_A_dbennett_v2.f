...
*
* r(CN) is average v=2 inernuclear separation, <2|r|2>
*
...
*
* revision of pot_cnar_A_dbennett_v4 for v=2
* 1-dec-2016 (p. dagdigiab)
*
...
*
      potnam='Ar-CN(A,v=2) RCCSD(T) PES D. Bennett'
      econv=219474.6d0
      print *, potnam
1      print *, ' r (bohr)'
      rshift=0.5
...
99    rr=3.5d0
      dr=0.5d0
      open (unit=12,file='cnar_Av2_dbennett_vlms.txt')
      write(12,109)
...
* ------------------------------------------------------------------------
      ...
      common /pointsdif/rdif(nptdif),rh2dif(nptdif),thetadif(nptdif)
      common /coefdif/vcoefdif(nptdif)
      potnam='Ar-CN(A,r=<2|r|2>) UCCSD(T) PES D. Bennett'
      lammin(1)=1
      lammax(1)=10
      lammin(2)=2
      lammax(2)=10
...
C 2.3768 is r-distance averaged with v=4 of CN(A) 
C vibrational wavefunction
      do 100 i=1,11
        vsum(i)=vsum_3D(2.3768D0,r,thta(i),vcoefsum)
* don't compute vdif for colinear geometries
        if (i.ne.1 .and. i.ne.11) then
          vdif(i-1)=vdif_3D(2.3768D0,r,thta(i),vcoefdif)
