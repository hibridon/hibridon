...
where
  ...
  ⚠️ In order to create the {jobname}.wfu file, the previous calculation must have been carried out with the flags [WAVEFL](WAVEFL) and [AIRYFL](AIRYFL-and-PRAIRY) both set .TRUE.
...
If `IFLUX = −1,1, or −2` then the {additional variables} are `THRESH`, `IPRINT`, `DAMP`, `INITIAL-J`, `INITIAL-L`, and `INITIAL-IND`. These are defined as follows:
  ...
  - In addition, channel fluxes are printed out ONLY for those channels which are specified in the arrays [JOUT](JOUT) and [INDOUT](INDOUT).
...
⚠️ Fluxes are determined at all values of R lying between [RENDLD](RSTART,-RENDLD,-and-RENDAI) and [RENDAI](RSTART,-RENDLD,-and-RENDAI), with the grid size and spacing controlled by the same parameters which govern the AIRY integration, namely [SPAC](SPAC), [FSTFAC](FSTFAC), [TOLAI](TOLAI-and-RINCR), and [RINCR](TOLAI-and-RINCR).
